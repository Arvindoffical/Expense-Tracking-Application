...
class ExpenseTracerApp:
    def __init__(self):
        ...
        self.connection = mysql.connector.connect(
            host="localhost", username="root", password="Arvind22061", database="registered_users_expense_tracer")
        # Email Entry Box
    ...
    def login_sucess(self, email):
        ...
        connection_with_indivisual_database = mysql.connector.connect(
            host="localhost", username="root", password="Arvind22061", database=f"{database_name}")
        ...
        def check_existing_user(email):
            try:
                ...
                # Ensure the email is properly quoted to prevent SQL injection
                query = "SELECT * FROM registered_users WHERE email = %s"
                cursor.execute(query, (email,))  # Use parameterized query to avoid SQL injection
                result = cursor.fetchone()

                if result:  # If a record is found, return True
                    return True
        ...
        def home():
            ...
            curMess_date = datetime.now()
            first_day = curMess_date.replace(day=1)
            next_month = (curMess_date.replace(day=28) +
        ...
        def add_expense():
            ...
            expense_category_dropdown = ctk.CTkComboBox(
                self.main_window_frame, width=160, values=["Mess", "Stationery", "Canteen", "Entertainment", "Other"], state="readonly")
            expense_category_dropdown.set("Mess")
        ...
        def visualize_expense():
            ...
            self.main_window_frame = ctk.CTkFrame(
                logged_in_window, height=500, width=450)
            self.main_window_frame.place(x=150, y=0)
            categories = ["Mess", "Stationery",
                          "Canteen", "Entertainment", "Other"]
            cursor = connection_with_indivisual_database.cursor()
            expenses = []
            query_to_find_total_expense_in_Mess = "select sum(expense_amount) from expenses where expense_category='Mess';"
            cursor.execute(query_to_find_total_expense_in_Mess)
            Mess_expense = cursor.fetchone()
            cursor.reset()
            expenses.append(Mess_expense)
            query_to_find_total_expense_in_Stationery = "select sum(expense_amount) from expenses where expense_category='Stationery';"
            cursor.execute(query_to_find_total_expense_in_Stationery)
            Stationery_expense = cursor.fetchone()
            cursor.reset()
            expenses.append(Stationery_expense)
            query_to_find_total_expense_Canteen = "select sum(expense_amount) from expenses where expense_category='Canteen';"
            cursor.execute(query_to_find_total_expense_Canteen)
            Canteen_expense = cursor.fetchone()
            cursor.reset()
            expenses.append(Canteen_expense)
        ...
        def delete_expense():
            ...
            search_by_category_combobox = ctk.CTkComboBox(self.main_window_frame, width=160, values=[
                                                          "Mess", "Stationery", "Canteen", "Entertainment", "Other"], state="readonly")
            search_by_category_combobox.set("Mess")
                    ...
                    if entered_otp == generatedotp:
                        if entered_password == confirm_password:
                            try:
                                ...
                                # Get the user input from the relevant entry fields
                                # user_name = self.user_name_entry.get()  # Get username from entry field
                                email = self.email_entry.get()  # Get email from entry field
                                password = self.password_define_entry.get()  # Get password from entry field

                                # Build database name if necessary (optional)
                                db = "".join(database_to_be_generated).replace('.com', '').replace('.', '')

                                # Parameterized query to insert data safely
                                query = "INSERT INTO registered_users (email, password) VALUES (%s, %s)"

                                # Execute the query with the user input values
                                cursor.execute(query, (email, password))
                                self.connection.commit()  # Commit the transaction

                                # Show success message and call login flow
                                ...
                                def login_after_signup():
                                    ...
                                    # Set up login fields
                                    self.password_define_entry.place(x=250, y=80)
                                    self.new_login_button = ctk.CTkButton(self.landing_window, text="Login", width=170,
                                                                          command=new_login_again)
                                    self.new_login_button.place(x=250, y=110)

                                    # Disable fields to prevent modification
                                    self.password_define_entry.configure(state="disabled")
                                    self.email_entry.configure(state="disabled")

                                # Create a button to trigger the login after signup
                                self.login_button_after_signup = ctk.CTkButton(self.landing_window, text="Login",
                                                                               command=login_after_signup, width=170)
                                self.login_button_after_signup.place(x=250, y=200)

                            except mysql.connector.IntegrityError as e:
                                if "1062" in str(e):  # Check for duplicate entry error
                                    messagebox.showerror("Account Already Created",
                                                         "Your Account has already been created, proceed towards login")
                                else:
                ...
                if len(entered_mail) != 0:
                    ...
                    if (len(result) == 1):
                        ...
                        try:
                            ...
                            connection.starttls()
                            connection.login(
                                "arvindgarg1107@gmail.com", "ssbp ptmi guwk vick")
                            otp = ""
                            for i in range(4):
                                otp += str(random.randrange(0, 9, 1))
                            connection.sendmail(
                                "arvindgarg1107@gmail.com", f"{entered_mail}", f"Subject: Regarding OTP \n\nYour One time Password is {otp}\nDo not share it with others\nif you have not requested for otp ignore it")
    ...
    def login_after_account_creation(self):
        try:
            ...
            self.connection_with_new_db = mysql.connector.connect(
                host="localhost", user="root", password="Arvind22061", database=f"{database_to_be_created}")
